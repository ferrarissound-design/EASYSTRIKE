...
export class Enemy {
  ...
  respawn(playerPosition){const candidates=[[-16,-15],[16,-15],[-16,15],[16,15],[0,-16]];const safe=candidates.map(([x,z])=>new THREE.Vector3(x,0,z)).sort((a,b)=>b.distanceToSquared(playerPosition)-a.distanceToSquared(playerPosition))[0];this.group.position.copy(safe);this.group.rotation.set(0,0,0);this.group.visible=true;
    ...
    this.hp=this.maxHp||100;this.alive=true;this.recoil=0;this.moving=false;this.animTime=0;this.moveDirection=null;this.moveSpeed=0;this.thinkTimer=1;this.shotTimer=1.2;this.seenTime=0;this.burstShots=0;this.visibleToPlayer=false;this.strafeDirection=Math.random()<.5?-1:1;this.effects?.ring(safe,0x7eeeff)}
}
