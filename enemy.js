...
export class Enemy {
  ...
  update(dt,player,onShot){this.animate(dt);if(!this.alive)return;this.thinkTimer-=dt;this.shotTimer-=dt;this.group.position.y=Math.max(0,this.group.position.y-8*dt);const toPlayer=player.position.clone().sub(this.group.position),distance=toPlayer.length(),visible=this.canSeePlayer(player);this.group.lookAt(player.position.x,this.group.position.y,player.position.z);if(this.thinkTimer>0)return;this.thinkTimer=.16;let direction=toPlayer.normalize(),speed=this.config.enemySpeed,goal='fight';const health=this.pickups?.nearestHealth(this.group.position);if(this.hp<=this.maxHp*.3&&health){direction=health.position.clone().sub(this.group.position).setY(0).normalize();goal='heal'}else if(this.hp<=this.maxHp*.3){direction.negate();goal='retreat'}else if(distance>11)goal='approach';else if(distance<7){direction.negate();goal='retreat'}else{direction.set(-direction.z,0,direction.x).multiplyScalar(this.strafeDirection)}if(!visible&&goal==='fight')goal='approach';const before=this.group.position.clone();this.safeMove(direction,speed*.16*(goal==='retreat'?1.2:1));this.moving=before.distanceToSquared(this.group.position)>1e-4;if(Math.random()<this.config.jump*.16&&this.group.position.y===0)this.group.position.y=.8;if(Math.random()<this.config.dash*.16)this.safeMove(direction,1.1);
    if(visible&&distance<25&&this.shotTimer<=0){if(this.burstShots>=3){this.shotTimer=1.3;this.burstShots=0;this.strafeDirection*=-1;return}this.shotTimer=this.config.reaction;this.burstShots++;this.seenTime+=.16;this.muzzleFlash();if(this.seenTime>this.config.reaction&&Math.random()<this.config.accuracy)onShot(this.config.attack);else onShot(0)}else if(!visible)this.seenTime=0}
