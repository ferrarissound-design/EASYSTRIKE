...
export class SupportDrone {
  ...
  update(dt,player,enemy){if(!this.active)return;const desired=player.position.clone().add(new THREE.Vector3(1.4,1,-.8).applyAxisAngle(new THREE.Vector3(0,1,0),player.yaw));this.group.position.lerp(desired,Math.min(1,dt*3));this.group.lookAt(enemy.group.position);this.healTimer-=dt;this.shotTimer-=dt;if(this.healTimer<=0){player.hp=Math.min(player.maxHp,player.hp+8);this.healTimer=5}if(enemy.alive&&this.shotTimer<=0&&this.group.position.distanceTo(enemy.group.position)<18){enemy.damage(3,{drone:true});this.effects.tracer(this.group.position,enemy.group.position.clone().add(new THREE.Vector3(0,2,0)),0x54dfff);this.shotTimer=3.5}}
}
